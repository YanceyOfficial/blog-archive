...
整体来讲, `createPluginContaienr` 有三大部分, 分别是 `Context`, `TransformContext` 和 `container`. 当 vite 在构建到某个阶段时, 会调用 `container` 中的函数, 在函数内遍历并执行所有插件对应的钩子函数, 而这些钩子函数是和 roullup 插件体系打通的. 而 `Context` 和 `TransformContext` 相当于一个工厂, 将 vite 的插件转换成 rollup 可用的.
...
class Context implements PluginContext {
  ...
  // parser 是 acorn 这个库, 用来将代码字符串转换为 AST
  // rollup, webpack 都用这个
